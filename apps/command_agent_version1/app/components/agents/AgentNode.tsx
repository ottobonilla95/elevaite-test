...
import { PenLine, Trash2, GripHorizontal } from "lucide-react";
...
const AgentNode = memo(({ id, data, selected }: NodeProps) => {
  ...
  const toolNames = tools.map(
    (tool: ChatCompletionToolParam) => tool.function.name
  );
  ...
  const shouldShowModelBadge = (): boolean => {
  ...
  return (
    ...
    >
      ...
      <div className="agent-node-header border-b-[1px] px-4 pb-4 items-start">
        <div className="agent-icon-container">
          <div className="agent-icon">{getAgentIcon(type)}</div>
      ...
      {/* Badge section */}
      {shouldShowModelBadge() && (
        <div className="agent-badges px-4 border-b-[1px] py-1">
