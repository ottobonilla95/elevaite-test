...
export default function VectorizerConfigPanel({
  ...
  // Removed expand functionality - not needed
    ...
    <div className="config-panel">
      ...
      <div className="config-panel-header">
        <div className="flex flex-1 items-center pr-3">
          ...
          {/* Debug button to manually show logs - remove this later */}
          {process.env.NODE_ENV === 'development' && (
            <button
              type="button"
              onClick={() => setShowLogs(!showLogs)}
              className="px-2 py-1 bg-blue-500 text-white text-xs rounded ml-2"
            >
      ...
      <div className="vectorizer-main-content">
        ...
        {process.env.NODE_ENV === 'development' && (
          <div style={{ position: 'fixed', top: '10px', right: '10px', background: 'yellow', padding: '5px', fontSize: '12px' }}>
            sidebarOpen: {sidebarOpen?.toString()}, showLogs: {showLogs.toString()}, isRunning: {isRunning.toString()}, progress: {progress}
          </div>
        )}
      </div>
    </div>
