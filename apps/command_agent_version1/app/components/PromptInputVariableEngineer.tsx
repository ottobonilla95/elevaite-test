import { CommonCheckbox, CommonSelect, type CommonSelectOption, SimpleInput, SimpleTextarea } from "@repo/ui/components"
import { useEffect, useState } from "react";
import { usePrompt } from "../ui/contexts/PromptContext";
import { type PromptInputVariableEngineerItem, type PromptInputVariableEngineerType } from "../lib/interfaces";
...
function PromptInputVariableEngineer(props: PromptInputVariableEngineerProps): JSX.Element {
	...
	const handleCancelEditVariable = (): void => {
		const selectedVariable = promptContext.promptInputVariablesEngineer.find(variable => variable.id === props.id);
		setName(selectedVariable?.name ?? "");
		setDisplayName(selectedVariable?.displayName ?? "");
		setType(selectedVariable?.type ?? "String");
	...
	const handleTypeChange = (text: PromptInputVariableEngineerType): void => {
		setType(text);
	}

	const handleRequiredChange = (value: boolean): void => {
		setRequired(value);
	}

	const handleJsonChange = (value: boolean): void => {
		setJson(value);
	}

	const handleDefinitionChange = (text: string): void => {
		setDefinition(text);
	}
				...
				<div className="card-variable prompt-input-container">
					<div className="prompt-input-header flex items-center justify-between" style={{ padding: '16px' }}>
						<div className="flex items-center gap-2">
							<div>{props.name}</div>
							<button onClick={handleEditVariable} title="Edit Variable" type="button">
						...
						<button onClick={handleRemoveVariable} title="Remove Variable" type="button">
					...
					{type === "String" && (
						<SimpleTextarea disabled wrapperClassName="prompt-input" value={definition} placeholder="{header / Customer / Product /Quantity / Invoice Amount}" useCommonStyling onChange={() => { console.log(""); }} style={{ filter: "none" }} />
					)}
					...
					<div className="rounded-lg bg-white" style={{ border: '1px solid #E3E8EF', margin: '5px' }}>
						<div className="p-4">
							<div className="header flex items-center justify-between pb-4" style={{ borderBottom: '1px solid #E3E8EF' }}>
								...
								<div className="flex items-center gap-2">
									<button onClick={handleSaveVariable} disabled={!allFilled} title="Save Variable" type="button">
									...
									{edit ? <button onClick={handleCancelEditVariable} title="Cancel Edit" type="button">
										<svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
											<path d="M11 1.5L1 11.5M1 1.5L11 11.5" stroke="#FF681F" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
										</svg>
									</button> : null}
									<button onClick={handleRemoveVariable} title="Remove Variable" type="button">
										<svg width="17" height="16" viewBox="0 0 17 16" fill="none">
								...
								<div>
									...
									{type === "String" && (
										<div className="prompt-input-container prompt-textarea-container no-margin">
											<SimpleTextarea wrapperClassName="prompt-input" value={definition} placeholder="{header / Customer / Product /Quantity / Invoice Amount}" useCommonStyling onChange={handleDefinitionChange} />
										</div>
									)}
									{type === "Array of Strings" && (
