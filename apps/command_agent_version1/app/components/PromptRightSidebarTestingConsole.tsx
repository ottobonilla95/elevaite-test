...
import { type PromptTestingConsoleInputTab } from '../lib/interfaces';
...
import "./PromptRightSidebarTestingConsole.scss";
...
function PromptRightSidebarTestingConsole(): JSX.Element {
	...
	function handleTabValueChange(index: number, newValue: string): void {
		...
		<div className={`bottom testing-console ${promptsContext.testingConsoleActiveClass} w-full rounded-b-xl overflow-y-auto`} style={{ height: `${String(height)}px` }}>
			...
			<div className="p-4 flex items-center justify-between bg-white sticky top-0 z-10">
				...
				<div className="flex gap-4 items-center">
					<button onClick={() => { promptsContext.setShowFileUploadModal(true); }} type="button">
			...
			<div className="tab-links-wrapper">
				<div className="tab-links px-4 flex gap-2">
					...
					<button className="font-medium text-sm px-4 py-3 flex items-center gap-2" onClick={handleAddTab} type="button">
								...
								<>
									<div className="flex items-center justify-between p-4" style={{ borderBottom: "1px solid #E2E8ED" }}>
										...
										<div className="text-sm font-medium flex-shrink-0" style={{ color: '#97A3B6' }}>
											{promptsContext.file.size ? `${(promptsContext.file.size / (1024 * 1024)).toFixed(2)} MB` : 'No file size'}
										</div>
									</div>
									...
									<div className="p-4">
										...
										<div className="my-4">
											{!promptsContext.invoiceImage ? undefined : (
