...
export function PromptInputEngineer(props: PromptInputProps): JSX.Element {
    const promptContext = usePrompt();

	const staticOptions = Object.entries(PromptInputTypes).map(([key, value]) => ({
		value,
		label: promptInputTypeLabels[value],
	}));

	const dynamicOptions = promptContext.promptInputVariablesEngineer.map(input => ({
	...
	useEffect(() => {
		const selectedVar = promptContext.promptInputVariablesEngineer.find(
			input => input.displayName === props.type
		);
		if (selectedVar) {
			if (text === "") {
				setText(selectedVar.definition || "");
				promptContext.updatePromptInput(props.id, { prompt: selectedVar.definition || "" });
			}
			if(values.length == 0) {
				setValues(selectedVar.values);
				promptContext.updatePromptInput(props.id, { values: selectedVar.values });
			}
		}

		const stillExists = promptContext.promptInputVariablesEngineer.some(
			input => input.displayName === props.type
		);
		if (!stillExists) {
			setText("");
    ...
    function handleTypeChange(value: string): void {
		const selectedVar = promptContext.promptInputVariablesEngineer.find(input => input.displayName === value);

		const newText = selectedVar ? (selectedVar.definition || "") : "";
    	const newValues = selectedVar ? selectedVar.values : [];

		setText(newText);
		setValues(newValues);
		promptContext.updatePromptInput(props.id, {
			prompt: newText,
			values: newValues,
			type: selectedVar ? selectedVar.displayName : value
		});
    }
    ...
    return (
        <div className="prompt-input-container">
			...
			{values && values.length > 0 ? (
            	...
            	<SimpleTextarea wrapperClassName="prompt-input" value={text} onChange={handleTextChange} placeholder="Enter prompt text..." useCommonStyling></SimpleTextarea>
			)}
        </div>
    );
}