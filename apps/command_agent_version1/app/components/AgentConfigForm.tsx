...
function AgentConfigForm(): JSX.Element {
	...
	useEffect(() => {
		if (mounted && lastSavedState !== "") { // Only check after we have a baseline
			const currentState = getCurrentWorkflowState();
	...
	const handleDeployWorkflow = async (nameOverride?: string) => {
		...
		try {
			...
			// Deploy the workflow
			const deploymentResponse = await deployWorkflowAndRefresh(workflowData.workflow_id, {
			...
			if (deploymentResponse && workflowData) {
	...
	const handleEditWorkflow = (name: string, description: string, tags: string[]) => {
	...
	const handleSaveWorkflowCallback = useCallback((name: string, _description: string, tags: string[]) => {
	...
	const handleDeployWorkflowCallback = useCallback((name: string, _description: string, tags: string[]) => {
	...
	const handleCreateNewWorkflowCallback = useCallback((name: string, _description: string, tags: string[]) => {
				...
				workflowDetails.workflow_connections.forEach(
					(connection, index: number) => {
						const newEdge: Edge = {
							...
							type: "custom",
							data: {
								actionType: mapConnectionTypeToActionType(connection.connection_type)
							}
						};
	...
	const handleUpdateExistingAgentInDB = async (
		...
		const updatedAgent = await updateAgentAndRefresh(selectedNode.data.agent.agent_id, agentUpdateData);
			...
			<ReactFlowProvider>
				<div className="agent-config-form" ref={reactFlowWrapper}>
					...
					{Boolean(showTestingSidebar) && (
						<AgentTestingPanel workflowId={currentWorkflowData?.workflow_id ?? ""} sessionId={currentWorkflowData?.id.toString()} />
					)}
				</div>
			</ReactFlowProvider>
		</>
