...
interface SortingObject<RowObjectType> {
  field: keyof RowObjectType;
  isDesc: boolean;
}
...
type ListRowProps<RowObjectType> =
  | ListNormalRow<RowObjectType>
  | ListHeaderRow<RowObjectType>;

export function ListRow<RowObjectType>(
  props: ListRowProps<RowObjectType>
): JSX.Element {
  function getSpecialItem(item: RowStructure<RowObjectType>): React.ReactNode {
    if (!item.specialHandling || !props.rowItem) return "";
    ...
    switch (item.specialHandling) {
        ...
        if (Array.isArray(roles)) {
          ...
          if (roles.length >= 1 && "roleLabel" in roles[0]) {
            ...
            if (
              ...
              verifiedRoles[0].roleLabel === "loading"
            )
              return (
            ...
            return (
              <div className="roles-container">
                {verifiedRoles.map((role) => (
                  <div
                    className="role"
                    key={role.roleLabel + (role.roleParent ?? "")}
                  >
                    <span className="label">{role.roleLabel}</span>
                    {!role.roleParent ? undefined : (
                      <>
                        <span>|</span>
                        <span className="parent">{role.roleParent}</span>
                      </>
                    )}
                  </div>
                ))}
      ...
      case specialHandlingListRowFields.PERMISSIONS: {
        ...
        return (
          <div
            className={["permission-field", permission ? "green" : "red"]
              .filter(Boolean)
              .join(" ")}
          >
            {permission ? "YES" : "NO"}
      ...
      case specialHandlingListRowFields.TAGS:
        return props.rowItem?.[item.field] ? (
          <>
            {(props.rowItem[item.field] as string[]).map((tag: string) => (
      ...
      case specialHandlingListRowFields.COUNT:
        return props.rowItem?.[item.field] !== undefined ? (
          <div className="count">{props.rowItem[item.field]}</div>
        ) : (
          ""
        );
      case specialHandlingListRowFields.EMAIL:
        return props.rowItem?.[item.field] ? (
          <a
            className="email"
            href={`mailto:${props.rowItem[item.field] as string}`}
          >
            {props.rowItem[item.field]}
          </a>
        ) : (
          ""
        );
      case specialHandlingListRowFields.DATE:
        return props.rowItem?.[item.field]
          ? dayjs(props.rowItem[item.field] as string).format(
            "DD-MMM-YYYY hh:mm a"
          )
          : "";
      case specialHandlingListRowFields.SHORT_DATE:
        return (
          <span
            title={
              props.rowItem?.[item.field]
                ? `${dayjs(props.rowItem[item.field] as string).format("DD-MMM-YYYY")}\n${dayjs(props.rowItem[item.field] as string).format("hh:mm a")}`
                : ""
            }
          >
            {props.rowItem?.[item.field]
  ...
  function handleRowClick(): void {
    if ("onClick" in props && props.onClick) props.onClick(props.rowItem);
  }
            ...
            {structureItem.isSortable ? (
              <div
                className={[
                  "sort-container",
                  props.sorting &&
                    "field" in props.sorting &&
                    typeof props.sorting.field === "string" &&
                    props.sorting.field === structureItem.field &&
                    props.sorting.isDesc
                    ? "desc"
                    : undefined,
                  props.sorting &&
                    "field" in props.sorting &&
          ...
          structureItem.onClick ? (
            <CommonButton
              ...
              className={[
                ...
                .join(" ")}
              onClick={() => {
                if (structureItem.onClick) structureItem.onClick(props.rowItem);
              }}
              disabled={!structureItem.isSortable}
              overrideClass
            >
              {structureItem.formattingFunction
                ? structureItem.formattingFunction(props.rowItem, props.index)
                : structureItem.specialHandling
                  ? getSpecialItem(structureItem)
                  : props.rowItem[structureItem.field]
                    ? props.rowItem[structureItem.field]
                    : ""}
            </CommonButton>
          ) : (
