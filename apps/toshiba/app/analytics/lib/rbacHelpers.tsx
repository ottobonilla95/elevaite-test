...
function getDisplayPermissionRecursively(
  ...
  const currentPermission: DisplayPermission = {
    entity:
      passedLabels && passedLabels.length > 0 ? passedLabels[0] : "Unknown",
  ...
  for (const key of Object.keys(permission)) {
    if (Object.values<string>(ROLE_PERMISSIONS).includes(key)) {
      ...
      switch (key as ROLE_PERMISSIONS) {
        case ROLE_PERMISSIONS.ACTION_CREATE:
          currentPermission.canCreate = permission[key] === "Allow";
          break;
        case ROLE_PERMISSIONS.ACTION_READ:
          currentPermission.canRead = permission[key] === "Allow";
          break;
        case ROLE_PERMISSIONS.ACTION_UPDATE:
          currentPermission.canUpdate = permission[key] === "Allow";
          break;
        case ROLE_PERMISSIONS.ACTION_DELETE:
          currentPermission.canDelete = permission[key] === "Allow";
          break;
      }
      ...
      displayPermissions.push({
        ...
        type:
          labels.length > 1
            ? [...labels.slice(1), getLabelFromKey(key)]
            : [getLabelFromKey(key)],
      ...
      displayPermissions.push(
        ...getDisplayPermissionRecursively(
          permission[key] as RolePermission,
          [...labels, getLabelFromKey(key)].filter(Boolean)
        )
      );
    }
  }
...
function getLabelFromKey(key: string): string {
  ...
  if (splitKey[0] !== "TYPENAMES" && splitKey[1]) {
    result =
      splitKey[1].charAt(0).toUpperCase() + splitKey[1].slice(1).toLowerCase();
  }
  return result;
}
