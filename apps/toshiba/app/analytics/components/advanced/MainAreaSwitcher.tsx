...
export function MainAreaSwitcher({ isSidebarCollapsed }): JSX.Element {
    ...
    useEffect(() => {
        ...
        const shouldAllow: boolean = allowedEmails.includes(chatContext.userEmail as string) || Boolean(chatContext.isBatchEvaluationUser);
    ...
    useEffect(() => {
        if (chatContext.setActiveWindowGrid === undefined) {
            getActiveWindow(WindowGrid.toshiba1);
        } else {
    ...
    return (
        <div className={["main-area-switcher-container", chatContext.setActiveWindowGrid ?? "toshiba1", isSidebarCollapsed ? "sidebar-collapsed" : "sidebar-expanded"].filter(Boolean).join(" ")}>
