...
export default function StepPage(): JSX.Element {
  ...
  const step = mediaIngestSteps.find((s) => s.id === stepId);

  if (!step) {
    return (
      <div className="step-not-found">
        <h1>Step not found</h1>
        <p>The requested media ingestion step does not exist.</p>
        <button onClick={() => router.push("/")}>
          Return to MediaIngest Overview
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="step-detail-container">
      ...
      <div className="step-detail-content">
        ...
        <div className="detail-section">
          <h2>Process Position</h2>
          <div className="pipeline-position">
            {mediaIngestSteps.map((s, index) => (
