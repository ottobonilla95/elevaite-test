...
import {
  costBarChartAxisValues,
  useCost,
} from "../../../lib/contexts/CostContext";
...
export function CostBarChart(): JSX.Element {
  ...
  function getFooter(tooltipItems: TooltipItem<"bar">[]): string {
    ...
    tooltipItems.forEach((tooltipItem: TooltipItem<"bar">) => {
      totalAmount += tooltipItem.parsed.y ?? 0;
    });
    switch (costContext.costBarChartAxis) {
      case costBarChartAxisValues.TOKENS:
        return `Total Tokens:    ${totalAmount.toLocaleString(undefined, {
          minimumFractionDigits: 0,
          maximumFractionDigits: 0,
        })}`;
      case costBarChartAxisValues.GPU:
        return `Total GPU Usage:    ${totalAmount.toLocaleString(undefined, {
          minimumFractionDigits: 0,
          maximumFractionDigits: 2,
        })} mins`;
      case costBarChartAxisValues.COST:
        return `Total Cost:    $ ${totalAmount.toLocaleString(undefined, {
  ...
  function formatChartYAxis(value: string): string {
    ...
    switch (costContext.costBarChartAxis) {
      case costBarChartAxisValues.TOKENS:
        return `${formattedValue} tkn`;
      case costBarChartAxisValues.GPU:
        return `${formattedValue} mins`;
      case costBarChartAxisValues.COST:
        ...
        <Bar
          data={{
            ...
            datasets: !costContext.costDetails.uniqueModels
              ? []
              : costContext.costDetails.uniqueModels.map((model) => {
                  return {
                    label: model,
                    data: costContext.getValueOfModelPerProject(
                      model,
                      costContext.costBarChartAxis,
                    ),
