...
import { AppInstanceStatus, PipelineStatus } from "../../../../lib/interfaces";
...
export function PreprocessPipelineWidget(props: PreprocessPipelineWidgetProps): JSX.Element {
    ...
    ): (PipelineStep & PipelineStepData)[][] {
        //TODO: @Thanos please check
        // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment -- Check
        const stepClone = JSON.parse(JSON.stringify(steps));
        //TODO: @Thanos please check
        // eslint-disable-next-line @typescript-eslint/no-unsafe-return -- Check
                            ...
                            {displaySteps.map((stepGroup, index) => (
                                <Fragment key={index}>
                                    ...
                                    <div className="step-group-container">
                                        {stepGroup.map((step) => (
                                            <StepBlock key={step.id} stepNumber={index + 1} {...step} />
                                        ))}
                                    </div>
                                </Fragment>
                            ))}
...
function StepBlock(props: (PipelineStep & PipelineStepData) & { stepNumber?: number }): JSX.Element {
    ...
    function getTimeTooltip(startTime: string, endTime?: string, status?: PipelineStatus): string {
        ...
        if (endTime) {
            tooltip += `\n${status === PipelineStatus.FAILED ? "Failed on" : "Completed on"}: ${dayjs(endTime)
                .utc()
                .format(formatting)} (utc)`;
        }
        return tooltip;
    }
