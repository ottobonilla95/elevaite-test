...
import { type ExtendedUserObject, type SortingObject } from "../../../lib/interfaces";
...
export function UsersList(props: UsersListProps): JSX.Element {
    ...
    const [sorting, setSorting] = useState<SortingObject<ExtendedUserObject>>({field: undefined});
    ...
    const usersListStructure: RowStructure<ExtendedUserObject>[] = [
        ...
        { header: "Email Address", field: "email", isSortable: true, specialHandling: specialHandlingListRowFields.EMAIL },
        { header: "Roles", field: "displayRoles", isSortable: false, specialHandling: specialHandlingListRowFields.ROLES },
    ];

    const usersListMenu: CommonMenuItem<ExtendedUserObject>[] = [
        { label: "Edit User name", onClick: (item: ExtendedUserObject) => { handleMenuClick(item, menuActions.EDIT_NAMES); } },
        { label: "Edit User roles", onClick: (item: ExtendedUserObject) => { handleMenuClick(item, menuActions.EDIT_ROLES); } },
    ]
    ...
    function arrangeDisplayUsers(): void {
        const usersClone = JSON.parse(JSON.stringify(rolesContext.users)) as ExtendedUserObject[];
        ...
        if (searchTerm) {
            for (const item of usersClone) {
                if (item.firstname?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
                if (item.lastname?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
                if (item.email?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
    ...
    function handleMenuClick(user: ExtendedUserObject, action: menuActions): void {
        switch (action) {
            case menuActions.EDIT_NAMES: handleEditUser(user); break;
            case menuActions.EDIT_ROLES: handleEditUserRoles(user); break;
            default: break;
        }
    }
            ...
            <div className="users-list-table-container">
                ...
                displayUsers.map((user, index) => 
                    <UserRolesListRow
                        ...
                        structure={usersListStructure}
                        menu={usersListMenu}
                        menuToTop={displayUsers.length > 4 && index > (displayUsers.length - 4) }
                    />
                )}
            </div>
