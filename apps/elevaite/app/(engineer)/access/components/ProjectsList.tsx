...
export function ProjectsList(props: ProjectsListProps): JSX.Element {
    ...
    const [displayProjects, setDisplayProjects] = useState<ExtendedProjectObject[]>([]);
    const [searchTerm, setSearchTerm] = useState("");
    const [sorting, setSorting] = useState<SortingObject<ExtendedProjectObject>>({field: undefined});
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [selectedProject, setSelectedProject] = useState<ExtendedProjectObject|undefined>();
    ...
    const projectsListStructure: RowStructure<ExtendedProjectObject>[] = [
        ...
        { header: "Creator", field: "creator", isSortable: true, specialHandling: specialHandlingListRowFields.EMAIL },
        { header: "Description", field: "description", isSortable: false },
    ];

    const projectsListMenu: CommonMenuItem<ExtendedProjectObject>[] = [
        { label: "Edit Project", onClick: (item: ExtendedProjectObject) => { handleMenuClick(item, menuActions.EDIT); } },
    ]
    ...
    function arrangeDisplayProjects(): void {
        const expandedProjects = getProjectsWithProjectDetails(rolesContext.projects);

        // Search
        const searchedList: ExtendedProjectObject[] = searchTerm ? [] : expandedProjects;
        if (searchTerm) {
            for (const item of expandedProjects) {
                ...
                if (item.accountName?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
                if (item.parentProjectName?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
                if (item.creator?.toLowerCase().includes(searchTerm.toLowerCase())) { searchedList.push(item); continue; }
    ...
    function handleMenuClick(project: ExtendedProjectObject, action: menuActions): void {
        switch (action) {
            case menuActions.EDIT: handleEditProject(project); break;
            default: break;
        }
    }


    function getProjectsWithProjectDetails(projects: ProjectObject[]): ExtendedProjectObject[] {
        const formattedProjects = JSON.parse(JSON.stringify(projects)) as ExtendedProjectObject[];

        for (const project of formattedProjects) {
            // Add account name
            if (project.account_id) {
                const foundAccount = rolesContext.accounts?.find(item => item.id === project.account_id);
                if (foundAccount) { project.accountName = foundAccount.name; };
            }
            // Add Parent Project name
            if (project.parent_project_id) {
                const foundParentProject = rolesContext.projects.find(item => item.id === project.parent_project_id);
                if (foundParentProject) { project.parentProjectName = foundParentProject.name; };
            }
        }
    ...
    return (
        <div className="projects-list-container">
            ...
            <ListHeader
                ...
                addIcon={<ElevaiteIcons.SVGProjects className="add-padding"/>}
            ...
            <div className="accounts-list-table-container">
                ...
                displayProjects.map((project, index) => 
                    <ListRow<ExtendedProjectObject>
                        ...
                        structure={projectsListStructure}
                        menu={projectsListMenu}
                        menuToTop={displayProjects.length > 4 && index > (displayProjects.length - 4) }
                    />
                )}
            </div>
