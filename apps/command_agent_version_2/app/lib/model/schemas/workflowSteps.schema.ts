...
export function isWorkflowConfig(data: unknown): data is WorkflowSteps.WorkflowConfig {
  ...
  if (data.timeout_seconds !== undefined && typeof data.timeout_seconds !== "number") return false;
  return true;
}

export function isWorkflowConfigArray(data: unknown): data is WorkflowSteps.WorkflowConfig[] {
  return Array.isArray(data) && data.every(isWorkflowConfig);
}

export function isStepExecutionResult(data: unknown): data is WorkflowSteps.StepExecutionResult {
  if (!isRecord(data)) return false;
  ...
  if (data.duration_ms !== undefined && typeof data.duration_ms !== "number") return false;
  return true;
}

export function isExecutionStatusResponse(data: unknown): data is WorkflowSteps.ExecutionStatusResponse {
  if (!isRecord(data)) return false;
  ...
  if (data.started_at !== undefined && typeof data.started_at !== "string") return false;
  if (data.completed_at !== undefined && typeof data.completed_at !== "string") return false;
  return true;
}

export function isExecutionResultsResponse(data: unknown): data is WorkflowSteps.ExecutionResultsResponse {
  if (!isRecord(data)) return false;
  if (typeof data.id !== "string") return false;
  if (typeof data.workflow_id !== "string") return false;
  if (typeof data.status !== "string") return false;
  if (data.output_data !== undefined && !isRecord(data.output_data)) return false;
  if (data.step_io_data !== undefined && !isRecord(data.step_io_data)) return false;
  if (data.started_at !== undefined && typeof data.started_at !== "string") return false;
  // if (data.completed_at !== undefined && typeof data.completed_at !== "string") return false;
  return true;
}
