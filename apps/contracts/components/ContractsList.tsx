...
const ContractsTabsArray: {
  label: CONTRACTS_TABS;
  tooltip?: string;
  isDisabled?: boolean;
}[] = [
...
function structureName(listItem: ContractObject): React.ReactNode {
  return (
    <span
      ...
      title={listItem.label ? listItem.filename : ""}
    >
...
function structureStatus(listItem: ContractObject): React.ReactNode {
  function statusSwitch(status: CONTRACT_STATUS): JSX.Element {
    switch (status) {
      case CONTRACT_STATUS.COMPLETED:
        return <ElevaiteIcons.SVGCheckmark />;
      case CONTRACT_STATUS.APPROVED:
        return <ElevaiteIcons.SVGCheckmark />;
      case CONTRACT_STATUS.PROCESSING:
        return <ElevaiteIcons.SVGInstanceProgress />;
      case CONTRACT_STATUS.FAILED:
        return <ElevaiteIcons.SVGXmark />;
  ...
  return (
    <div
      className={["contract-status", listItem.status].join(" ")}
      title={listItem.status}
    >
...
function structureApproval(listItem: ContractObject): React.ReactNode {
  const isApproved =
    listItem.status === CONTRACT_STATUS.APPROVED ||
    listItem.verification?.verification_status === true;
  return (
    <span
      className={["contract-approval", !isApproved ? "pending" : undefined]
        .filter(Boolean)
        .join(" ")}
    >
...
function structureContractNumber(listItem: ContractObject): React.ReactNode {
  switch (listItem.content_type) {
    case CONTRACT_TYPES.INVOICE:
      return listItem.highlight?.invoice_number ?? "";
    case CONTRACT_TYPES.PURCHASE_ORDER:
      return listItem.highlight?.po_number ?? "";
    default:
      return listItem.highlight?.contract_number ?? "";
  }
}
function structureVerification(
  listItem: ContractObject,
  checkingType: CONTRACT_TYPES
): React.ReactNode {
  let isVerified = false;

  switch (checkingType) {
    case CONTRACT_TYPES.INVOICE:
      isVerified = listItem.verification?.invoice?.length
        ? listItem.verification.invoice.some((item) => item.verification_status)
        : false;
      break;
    case CONTRACT_TYPES.PURCHASE_ORDER:
      isVerified = listItem.verification?.po?.length
        ? listItem.verification.po.some((item) => item.verification_status)
        : false;
      break;
    case CONTRACT_TYPES.VSOW:
      isVerified = listItem.verification?.vsow?.length
        ? listItem.verification.vsow.some((item) => item.verification_status)
        : false;
      break;
    case CONTRACT_TYPES.CSOW:
      isVerified = listItem.verification?.csow?.length
        ? listItem.verification.csow.some((item) => item.verification_status)
        : false;
      break;
  }
...
export function ContractsList({
  ...
  const [selectedTab, setSelectedTab] = useState(
    CONTRACTS_TABS.SUPPLIER_CONTRACTS
  );
  ...
  function getFilterTags(contractsWithTags: ContractObject[]): FilterTag[] {
    const uniqueTags = Array.from(
      new Set(contractsWithTags.flatMap((contract) => contract.tags ?? []))
    );
  ...
  function getRowListStructure(
    ...
    switch (tab) {
      case CONTRACTS_TABS.SUPPLIER_INVOICES:
        structure.push({
          ...
          isSortable: true,
          formattingFunction: structureContractNumber,
        });
        break;
    ...
    structure.push({
      ...
      field: "tags",
      isSortable: false,
      specialHandling: specialHandlingListRowFields.TAGS,
    });
    ...
    if (selectedTab === CONTRACTS_TABS.SUPPLIER_INVOICES)
      structure.push({
        ...
        field: "approval",
        isSortable: true,
        formattingFunction: structureApproval,
      });
    ...
    switch (tab) {
      case CONTRACTS_TABS.SUPPLIER_INVOICES:
        structure.push({
          header: "VSOW",
          field: "inv_vsow_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.VSOW),
        });
        structure.push({
          header: "PO",
          field: "inv_po_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.PURCHASE_ORDER),
        });
        break;
      case CONTRACTS_TABS.SUPPLIER_POS:
        structure.push({
          header: "VSOW",
          field: "po_vsow_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.VSOW),
        ...
        structure.push({
          header: "Inv.",
          field: "po_inv_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.INVOICE),
        });
        break;
      case CONTRACTS_TABS.CUSTOMER_CONTRACTS:
        structure.push({
          header: "VSOW",
          field: "csow_vsow_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.VSOW),
      ...
      case CONTRACTS_TABS.SUPPLIER_CONTRACTS:
        structure.push({
          header: "CSOW",
          field: "vsow_csow_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.CSOW),
        });
        structure.push({
          header: "Inv.",
          field: "vsow_inv_verification",
          align: "center",
          isSortable: false,
          formattingFunction: (item) =>
            structureVerification(item, CONTRACT_TYPES.INVOICE),
  ...
  function getTabContracts(
    ...
    switch (tab) {
      case CONTRACTS_TABS.CUSTOMER_CONTRACTS:
        contractType = CONTRACT_TYPES.CSOW;
        break;
      case CONTRACTS_TABS.SUPPLIER_CONTRACTS:
        contractType = CONTRACT_TYPES.VSOW;
        break;
      case CONTRACTS_TABS.SUPPLIER_INVOICES:
        contractType = CONTRACT_TYPES.INVOICE;
        break;
      case CONTRACTS_TABS.SUPPLIER_POS:
        contractType = CONTRACT_TYPES.PURCHASE_ORDER;
        break;
    }
    return allContracts.filter(
      (contract) => contract.content_type === contractType
    );
  }

  function getTaggedContracts(
    tabbedContracts: ContractObject[],
    filters: FilterTag[]
  ): ContractObject[] {
    ...
    const activeTags = filters
      ...
      .map((tag) => tag.label);
    return tabbedContracts.filter((contract) =>
      activeTags.some((tag) => contract.tags?.includes(tag))
    );
  }

  function getCrossMatches(
    contractsToCheck: ContractObject[]
  ): CrossMatchFields {
    const invoiceFailures = contractsToCheck.filter(
      (item) =>
        item.content_type === CONTRACT_TYPES.INVOICE &&
        item.verification?.verification_status === false
    ).length;
    const poFailures = contractsToCheck.filter(
      (item) =>
        item.content_type === CONTRACT_TYPES.PURCHASE_ORDER &&
        item.verification?.verification_status === false
    ).length;
    const vsowFailures = contractsToCheck.filter(
      (item) =>
        item.content_type === CONTRACT_TYPES.VSOW &&
        item.verification?.verification_status === false
    ).length;
    const csowFailures = contractsToCheck.filter(
      (item) =>
        item.content_type === CONTRACT_TYPES.CSOW &&
        item.verification?.verification_status === false
    ).length;
  ...
  function handleFilterTagClick(tag: FilterTag): void {
    setFilterTags((current) =>
      current.map((currentTag) =>
        currentTag.label === tag.label
          ? { ...currentTag, isActive: !tag.isActive }
          : currentTag
      )
    );
  }
  ...
  function handleRowClick(contract: ContractObject): void {
    router.push(`/${selectedProjectId}/${contract.id}`);
  ...
  return (
    <div className="contracts-list-container">
      <div
        className={[
          "cross-match-container",
          !selectedProjectId ? "concealed" : undefined,
        ]
          .filter(Boolean)
          .join(" ")}
      >
        <CrossMatchBit
          value={crossMatches?.[CONTRACT_TYPES.VSOW]}
          label="VSOW matches failed"
        />
        <CrossMatchBit
          value={crossMatches?.[CONTRACT_TYPES.CSOW]}
          label="CSOW matches failed"
        />
        <CrossMatchBit
          value={crossMatches?.[CONTRACT_TYPES.PURCHASE_ORDER]}
          label="PO matches failed"
        />
        <CrossMatchBit
          value={crossMatches?.[CONTRACT_TYPES.INVOICE]}
          label="Total invoices pending approval"
        />
      </div>

      <div className="tabs-container">
        {ContractsTabsArray.map(
          (item: {
            label: CONTRACTS_TABS;
            tooltip?: string;
            isDisabled?: boolean;
      ...
      <div className="contracts-list-table-container">
        {!selectedProjectId ? (
          ...
          <>
            ...
            <div
              className={[
                "contracts-list-table-contents",
                selectedTab === CONTRACTS_TABS.SUPPLIER_INVOICES
                  ? "invoice"
                  : selectedTab === CONTRACTS_TABS.SUPPLIER_CONTRACTS
                    ? "vsow"
                    : undefined,
              ]
                .filter(Boolean)
...
function CrossMatchBit(props: CrossMatchBitProps): JSX.Element {
  return (
    <div
      className={[
        "cross-match-bit-container",
        props.value !== undefined && props.value === 0 ? "success" : undefined,
      ]
        .filter(Boolean)
        .join(" ")}
    >
