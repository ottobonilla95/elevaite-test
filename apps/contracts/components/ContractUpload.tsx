...
export function ContractUpload(props: ContractUploadProps): JSX.Element {
  ...
  function handleFileChange(event: React.ChangeEvent<HTMLInputElement>): void {
    if (event.target.files?.[0]) setUploadingFile(event.target.files[0]);
    else setUploadingFile(undefined);
  }

  function handleSubmit(): void {
    if (!uploadingFile || !pdfType || !contractsContext.selectedProject?.id)
      return;
    contractsContext.submitCurrentContractPdf(
      uploadingFile,
      pdfType,
      contractsContext.selectedProject.id,
      name
    );
    props.onClose();
  }
  ...
  return (
    <div className="contract-upload-container">
      ...
      <div className="top-row">
        <CommonInput label="Name" onChange={setName} />

        <CommonFormLabels label="Choose File Type" required>
          <CommonSelect
            options={contractTypesOptions}
            onSelectedValueChange={handleContractTypeChange}
            defaultValue={
              props.selectedTab === CONTRACTS_TABS.SUPPLIER_INVOICES
                ? CONTRACT_TYPES.INVOICE
                : props.selectedTab === CONTRACTS_TABS.SUPPLIER_POS
                  ? CONTRACT_TYPES.PURCHASE_ORDER
                  : props.selectedTab === CONTRACTS_TABS.CUSTOMER_CONTRACTS
                    ? CONTRACT_TYPES.CSOW
                    : CONTRACT_TYPES.VSOW
            }
