...
class TestDependencies:
    ...
    @pytest.mark.db
    def test_get_tenant_db_dependency(
        self, fastapi_app, multitenancy_settings, setup_db, init_db_config
    ):
    ...
    def test_tenant_validator(self, fastapi_app, multitenancy_settings):
        ...
        validator = TenantValidator(
            tenant_callback=validate_tenant, settings=multitenancy_settings
        )
        ...
        response = client.get(
            "/test-validator", headers={"X-Tenant-ID": "valid-tenant"}
        )
        assert response.status_code == status.HTTP_200_OK
        ...
        response = client.get(
            "/test-validator", headers={"X-Tenant-ID": "invalid-tenant"}
        )
