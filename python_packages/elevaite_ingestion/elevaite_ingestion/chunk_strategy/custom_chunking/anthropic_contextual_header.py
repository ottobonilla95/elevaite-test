...
def generate_contextual_header(chunk: Dict, full_paragraphs: List[Dict], total_pages: int) -> str:
    try:
...
async def generate_contextual_header_async(chunk: Dict, full_paragraphs: List[Dict], total_pages: int) -> str:
    try:
        if not full_paragraphs:
            return "Header Generation Failed: Empty document"
        if any(p["page_no"] < 1 for p in full_paragraphs):
