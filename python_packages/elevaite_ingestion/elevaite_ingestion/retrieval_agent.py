...
def parse_tool_selection(doc_type):
    tools = config.get("parsing", {}).get("parsers", {}).get(doc_type, {}).get("available_tools", [])
    if not tools:
        return {
...
def get_load_agent(): return load_agent
def get_parse_agent(): return parse_agent
def get_chunk_agent(): return chunk_agent
def get_embedding_agent(): return embedding_agent
def get_vectorstore_agent(): return vectorstore_agent
