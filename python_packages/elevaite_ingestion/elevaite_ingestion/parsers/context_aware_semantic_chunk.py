...
def get_embeddings(texts: List[str]) -> List[List[float]]:
    ...
    try:
        ...
        ]  # Return zero vectors to maintain shape consistency
...
def semantic_chunking(sentences, threshold=0.8, force_split_on_page_change=True):
    ...
    for i, sentence in enumerate(sentences):
        if i > 0:
            ...
            if (
                force_split_on_page_change
                and sentences[i - 1].page_no != sentence.page_no
            ):
    ...
    if current_chunk:
        chunks.append(
            {"text": " ".join(current_chunk), "pages": sorted(list(chunk_page_numbers))}
        )
