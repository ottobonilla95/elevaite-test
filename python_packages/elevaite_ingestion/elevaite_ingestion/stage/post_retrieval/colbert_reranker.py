...
def colbert_rerank(query: str, chunks: List[Dict], top_k: int = 3) -> List[Dict]:
    ...
    print(
        "⚠️ NOTE: This is a simulated reranker. True reranking requires ColBERT pre-indexed corpus."
    )
    ...
    for result in results:
        matched = next(
            (
                chunk
                for chunk in chunks
                if result["text"].strip() in chunk["chunk_text"]
                and chunk["chunk_id"] not in seen_ids
            ),
            None,
        )
        if matched:
