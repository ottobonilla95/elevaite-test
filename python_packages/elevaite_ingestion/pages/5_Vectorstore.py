...
if selected_vector_db != "Select a Vector DB":
    ...
    if st.button("🚀 Run Vectorstore Pipeline"):
        ...
        if selected_vector_db == "qdrant":
            st.markdown("##### 🖥️ Visualize your collection")
            ...
            st.markdown(f"🔗 [Click here to view Qdrant Collection]({qdrant_dashboard_url})", unsafe_allow_html=True)
        if selected_vector_db == "pinecone":
            st.markdown("##### 🖥️ Visualize your Pinecone Index")
            ...
            st.markdown(f"🔗 [Click here to view Pinecone Index]({pinecone_dashboard_url})", unsafe_allow_html=True)
