...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        # Protocol version
        sa.Column(
            "protocol_version", sqlmodel.sql.sqltypes.AutoString(), nullable=True
        ),
        # Authentication
        sa.Column(
            "auth_type",
        ...
        sa.Column(
            "health_check_interval", sa.Integer(), nullable=False, server_default="300"
        ),
        # Status & Health
        sa.Column(
            "status",
    ...
    op.create_index(
        "ix_a2a_agents_organization_id", "a2a_agents", ["organization_id"], unique=False
    )
