        ...
        steps=[
            ...
            StepBase(
                ...
                tool_override=ToolOverride(
                    title="Data Processor", description="Process incoming data"
                ),
            ),
            StepBase(
                step_id="step-2",
...
def test_retrieve_and_update_workflow_ui_metadata(session: Session):
    ...
    config.connections.append(
        StepConnection(
            source_step_id="step-1", target_step_id="step-2", connection_type="default"
        )
    )
...
def test_workflow_without_ui_metadata(session: Session):
    ...
    workflow = Workflow(
        name=config.name, configuration=config.model_dump(), user_id="test-user-123"
    )
...
def test_complex_workflow_with_all_features(session: Session):
    ...
    config = WorkflowConfig(
        ...
        connections=[
            StepConnection(
                source_step_id="order-trigger", target_step_id="validate", animated=True
            ),
            StepConnection(
