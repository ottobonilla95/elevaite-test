...
class TestListApprovalRequests:
    ...
    def test_list_approval_requests_no_filters(
        self, mock_db_class, mock_session, sample_approval
    ):
    ...
    @patch("workflow_core_sdk.services.approvals_service.DatabaseService")
    def test_list_approval_requests_with_execution_id(
        self, mock_db_class, mock_session, sample_approval
    ):
    ...
    @patch("workflow_core_sdk.services.approvals_service.DatabaseService")
    def test_list_approval_requests_with_status(
        self, mock_db_class, mock_session, sample_approval
    ):
        ...
        approvals = ApprovalsService.list_approval_requests(
            mock_session, status="pending", limit=100, offset=0
        )
    ...
    @patch("workflow_core_sdk.services.approvals_service.DatabaseService")
    def test_list_approval_requests_with_all_filters(
        self, mock_db_class, mock_session, sample_approval
    ):
...
class TestGetApprovalRequest:
    ...
    def test_get_approval_request_success(
        ...
        approval = ApprovalsService.get_approval_request(
            mock_session, sample_approval["id"]
        )

        assert approval is not None
        assert approval["id"] == sample_approval["id"]
        assert approval["status"] == "pending"
        mock_db.get_approval_request.assert_called_once_with(
            mock_session, sample_approval["id"]
        )
...
class TestUpdateApprovalRequest:
    ...
    def test_update_approval_request_success(self, mock_db_class, mock_session):
        ...
        result = ApprovalsService.update_approval_request(
            mock_session, approval_id, changes
        )

        assert result is True
        mock_db.update_approval_request.assert_called_once_with(
            mock_session, approval_id, changes
        )
