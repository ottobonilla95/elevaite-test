...
class StreamManager:
    ...
    def remove_execution_stream(self, execution_id: str, queue: asyncio.Queue) -> None:
        ...
        logger.info(f"=== REMOVE_EXECUTION_STREAM CALLED for {execution_id} ===")
        if execution_id in self.execution_streams:
            ...
            if not self.execution_streams[execution_id]:
                ...
                logger.info(f"=== EXECUTION STREAM REGISTRY DELETED for {execution_id} ===")
        logger.info(f"=== Removed execution stream for {execution_id} ===")
