...
class RBACProvider:
    ...
    def _populate_rbac_schema_info(
        ...
        | None = None,
        entity_actions_to_path_params: dict[
            tuple[Type[models.Base], tuple[str, ...]], set[Type[models.Base]]
        ]
        | None = None,
        leaf_action_path_map: dict | None = None,
        entity_to_typenames_set: dict[Type[models.Base] | None, set] | None = None,
        entity_to_typenames_list: dict[Type[models.Base] | None, list[str]]
        | None = None,
        entity_to_typevalues_set: dict[Type[models.Base] | None, set] | None = None,
        entity_to_typevalues_list: dict[Type[models.Base] | None, list[tuple[str, ...]]]
        | None = None,
    ):
        ...
        for key, value in current_schema_obj.items():
            ...
            if key.startswith("ENTITY_"):  # model name delimited with 'ENTITY_' prefix
                ...
                if not isinstance(value, dict):  # if this is a leaf action
                    ...
                    if current_entity not in valid_entity_actions_map:
                ...
                raise ValueError(
                    "Not all of schema keys are prefixed with one of 'ENTITY_', 'TYPENAMES_', 'TYPEVALUES_', 'ACTION_'"
                )
    ...
    async def evaluate_rbac_permissions(
        ...
        model_class_to_instance = await self._map_model_class_to_instances(
            db, model_classStr_to_instanceIds
        )
        ...
        if (
            ...
            model_class_to_instance.update(
                await self._map_model_class_to_instances(
                    db, {"Account": model_class_to_instance[models.Project].account_id}
                )
            )
        ...
        logged_in_user_account_and_project_associations: dict[
            str, Any
            ...
            try:
                match field_name:
                    case "IS_PROJECT_ADMIN":
                        if models.Project not in model_class_to_instance:
                        ...
                        response_data[field_name] = {
                            "OVERALL_PERMISSIONS": logged_in_user_project_association.is_admin
                            if logged_in_user_project_association
                            else False
                        }
                    case "IS_ACCOUNT_ADMIN":
                        if models.Account not in model_class_to_instance:
                            raise ApiError.validationerror(
                        ...
                        response_data[field_name] = {
                            "OVERALL_PERMISSIONS": logged_in_user_account_association.is_admin
                    ...
                    case _:
                        ...
                        if not target_model_class:
                            print(
                                f"in rbac.validate_rbac_permissions : no target class found for target_model_class_str = {target_model_class_str}"
                            )
                        ...
                        if (
                            ...
                            not in self._account_scoped_permissions_valid_entity_actions_map[
                                target_model_class
                            ]
                        ):
                            print(
                                f'in rbac.validate_rbac_permissions : target model action sequence - "{target_model_action_sequence}" - not found in valid_entity_actions_map'
                            )
                            raise ApiError.serviceunavailable(
                                "The server is currently unavailable, please try again later."
                            )
                        ...
                        if (
                            auth_schemas.PermissionsEvaluationRequest.validate_permission_scope(
                                field_name,
                                auth_schemas.RBACPermissionScope.ACCOUNT_SCOPE,
                            )
                            and auth_schemas.PermissionsEvaluationRequest.validate_permission_scope(
                                field_name,
                                auth_schemas.RBACPermissionScope.PROJECT_SCOPE,
                            )
                        ):  # if endpoint supports account scoped and project scoped actions
                            if (
                                not logged_in_user.is_superadmin
                                and models.Account not in model_class_to_instance
                            ):
                                raise ApiError.validationerror(
                            ...
                            if not logged_in_user.is_superadmin:
                                ...
                                if (
                                    ...
                                    and models.Project not in model_class_to_instance
                                ):
                                    raise ApiError.validationerror(
                        ...
                        field_model_classStr_to_instanceIds = (
                            self._map_model_classStr_to_instanceIds(field_value)
                        )
                        field_model_class_to_instance = (
                            await self._map_model_class_to_instances(
                                db, field_model_classStr_to_instanceIds
                            )
                        )
                        self._validate_inter_model_associations(
                        ...
                        if typenames and typevalues:
                            ...
                            for typevalues_combination in typevalues:
                                ...
                                specific_permissions[typenames_key][typevalues_key] = (
                                    True
                                )
                        ...
                        if specific_permissions:
                            response_data[field_name]["SPECIFIC_PERMISSIONS"] = (
                                specific_permissions
                            )
    ...
    async def validate_rbac_permissions(
        ...
        if (
            "account_id" not in request.path_params
            and "project_id" not in request.path_params
        ...
        if (
            "project_id" not in request.path_params
        ...
        if (
            ...
            and "account_id" not in account_and_project_params
        ...
        logged_in_entity_account_and_project_associations: dict[
            str, Any
        ] = await self._validate_logged_in_entity_account_and_project_associations(
        ...
        model_classStr_to_instanceIds.update(
            self._map_model_classStr_to_instanceIds(params)
        )
        model_class_to_instance.update(
            await self._map_model_class_to_instances(db, model_classStr_to_instanceIds)
        )
        ...
        for param, value in params.items():
            if param.endswith("_id"):  # Checks if the parameter name indicates an id
                ...
                if (
                    ...
                    print(
                        f"in rbac._map_model_classStr_to_instanceIds : Invalid model class - '{model_classStr_candidate}' - derived from parameter: {param}"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
    ...
    async def _map_model_class_to_instances(
        ...
        for model_classStr, model_instance_id in model_classStr_to_instanceIds.items():
            ...
            if not model:
                print(
                    f"in rbac._map_model_class_to_instances : No model defined for model class: {model_classStr}"
                )
                raise ApiError.serviceunavailable(
                    "The server is currently unavailable, please try again later."
                )
                ...
                if not logged_in_user_account_association:
                    if not logged_in_user.is_superadmin:
                        raise ApiError.forbidden(
                            f"you are not assigned to account - '{model_class_to_instance[models.Account].id}'"
                        )
            ...
            if models.Project in model_class_to_instance:
                ...
                if not logged_in_user_project_association:
                    if not logged_in_user.is_superadmin and not (
                        ...
                        raise ApiError.forbidden(
                            f"you are not assigned to project - '{model_class_to_instance[models.Project].id}'"
                        )
                ...
                if not logged_in_user.is_superadmin and not (
                    ...
                    if parent_project_id and not is_user_project_association_till_root(
                        ...
                        raise ApiError.forbidden(
                            f"you are not assigned to one or more projects in the project hierarchy of parent project - '{parent_project_id}'"
                        )
        else:  #  authenticated entity is Apikey
            logged_in_apikey: models.Apikey = authenticated_entity
            if (
                models.Project not in model_class_to_instance
                or model_class_to_instance[models.Project].id
                != logged_in_apikey.project_id
            ):
                raise ApiError.forbidden(
                    f"your permissions are restricted to resources within project - '{logged_in_apikey.project_id}'"
                )
    ...
    async def _validate_account_and_project_based_permissions(
        ...
        if isinstance(logged_in_entity, models.User):
            ...
            if models.Account not in model_class_to_instance:
                raise ApiError.forbidden(
                    "you do not have superadmin permissions and must provide an account_id"
                )
            ...
            if model_class in model_class_to_instance:
                ...
                if isinstance(logged_in_entity, models.User):
                    ...
                    if logged_in_user_project_association:
                        ...
                        except Exception:  # field doesn't exist in project scoped rbac permission schema but it did in account scoped rbac permission schema, so skip it
                    ...
                    except Exception:  # current permission field doesn't exist in ApiKeyScopedRBACPermission schema; assume that it is account-scoped/project-scoped model eventually leading up to an api-key scoped target model-action sequence and skip. If not, permissions will be denied when target is evaluated.
            ...
            if target_model_class in self._entity_typenames_list:
                ...
                for type_values in self._entity_typevalues_list[target_model_class]:
                    ...
                    if isinstance(logged_in_entity, models.User):
                        ...
                        except Exception:  # target permission field doesn't exist in ApiKeyScopedRBACPermission schema; restrict project-scoped permissions
                            path_exists_in_project_scoped_permissions = False
                            if validation_info_key in permission_validation_info:
                                permission_validation_info[validation_info_key][
                ...
                if isinstance(logged_in_entity, models.User):
                    ...
                    if await self._check_project_scoped_permission_overrides_exist(
                        logged_in_entity_account_and_project_association_info,
                        target_model_action_permission_path,
                    ):
                        raise ApiError.forbidden(
    ...
    async def _check_project_scoped_permission_overrides_exist(
        ...
        if isinstance(logged_in_entity, models.User):
            ...
            try:
                ...
                print(
                    f"Invalid Project_scoped_permission overrides schema for user - '{logged_in_user_project_association.user_id}' - in project - '{logged_in_user_project_association.project_id}'"
                )
                raise e
            ...
            try:
                ...
                print(
                    f"Invalid Project_scoped_permission overrides schema for apikey - '{logged_in_entity.id}' - in project - '{logged_in_entity.project_id}'"
                )
                raise e
        if project_scoped_permission_overrides:
            ...
            for next_attribute in permission_path:
                if hasattr(current_attribute, next_attribute):
                    ...
                    print(
                        f"in check_project_scoped_permission_overrides_exist : logged-in {'user' if logged_in_entity_is_user else 'apikey'} - '{logged_in_entity.id}' - has malformed project permission overrides under project - '{logged_in_entity.project_id}'"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
    ...
    def _get_model_permissions_path(
        ...
        if permissions_scope is auth_schemas.RBACPermissionScope.ACCOUNT_SCOPE:
            if (
                ...
                print(
                    f"action sequence - '{target_model_action_sequence}' - for {target_model_class} resource not found in account-scoped-permissions schema"
                )
                raise ApiError.serviceunavailable(
                    "The server is currently unavailable, please try again later."
                )
            if (
                paths_map_input_tuple
                not in self._account_scoped_permissions_leaf_action_paths_map
            ):
                print(
                    f"action path not found in account-scoped-permissions schema for target model class - '{target_model_class}' - with model class sequence - '{model_class_sequence}' - with model_type_values_sequence - '{model_typevalues_sequence}' - with target_model_action_sequence - '{target_model_action_sequence}'"
                )
                raise ApiError.serviceunavailable(
                    "The server is currently unavailable, please try again later."
                )
            ...
            if isinstance(authenticated_entity, models.User):
                if (
                    target_model_action_sequence
                    not in self._project_scoped_permissions_valid_entity_actions_map[
                        target_model_class
                    ]
                ):
                    print(
                        f"action sequence - '{target_model_action_sequence}' - for {target_model_class} resource not found in project-scoped-permissions schema"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
                if (
                    paths_map_input_tuple
                    not in self._project_scoped_permissions_leaf_action_paths_map
                ):
                    print(
                        f"action path not found in project-scoped-permissions schema for target model class - '{target_model_class}' - with model class sequence - '{model_class_sequence}' - with model_type_values_sequence - '{model_typevalues_sequence}' - with target_model_action_sequence - '{target_model_action_sequence}'"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
                ...
                if (
                    target_model_action_sequence
                    not in self._apikey_scoped_permissions_valid_entity_actions_map[
                        target_model_class
                    ]
                ):
                    print(
                        f"action sequence - '{target_model_action_sequence}' - for {target_model_class} resource not found in apikey-scoped-permissions schema"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
                if (
                    paths_map_input_tuple
                    not in self._apikey_scoped_permissions_leaf_action_paths_map
                ):
                    print(
                        f"action path not found in apikey-scoped-permissions schema for target model class - '{target_model_class}' - with model class sequence - '{model_class_sequence}' - with model_type_values_sequence - '{model_typevalues_sequence}' - with target_model_action_sequence - '{target_model_action_sequence}'"
                    )
                    raise ApiError.serviceunavailable(
                        "The server is currently unavailable, please try again later."
                    )
            ...
            print(
                f"unimplemented business logic for get_model_permissions_path for scope - '{permissions_scope}'"
            )
            raise ApiError.serviceunavailable(
                "The server is currently unavailable, please try again later."
            )
    ...
    def _get_model_permission_validation_info(
        ...
        if (
            ...
            print(
                f"undefined action sequence - '{model_action_sequence}' - for {target_model_class} resource"
            )
            raise ApiError.serviceunavailable(
                "The server is currently unavailable, please try again later."
            )
    ...
    def get_instance(cls):
        if cls._instance is None:
            raise ValueError(
                "RBACProvider not initialized. Call the constructor first."
            )
        return cls._instance
