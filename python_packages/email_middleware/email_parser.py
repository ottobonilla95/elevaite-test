...
class EmailConversationParser:
    def get_email_conversations(self, email_content):
        ...
        email_content_without_footers = self.remove_footers(
            email_content_without_header
        )
        email_content_paragraphs_list = self.get_paragraphs(
            email_content_without_footers
        )
        ...
        all_email_conversations_without_header = [
            self.remove_conversation_header(conversation)
            for conversation in all_email_conversations
        ]
    ...
    def remove_conversation_header(self, conversation):
        ...
        for line in conv_lines:
            if re.match(
                email_regex_rules.email_conversation_header_regex, line.strip()
            ) or re.match(
                email_regex_rules.email_seperator_regex, line.strip().lower()
            ):
                pass
            else:
    ...
    def remove_footers(self, string_email, cosine_distance_threshold=0.9):
        ...
        list_paragraphs = [
            paragraph.strip()
            for paragraph in str(string_email).split("\n\n")
        ...
        for paragraph_number in range(len(list_paragraphs)):
            ...
            list_lines_footer = re.split(
                RegexRules().sentence_seperators, string_footer
            )
    ...
    def get_emails(self, list_paragraphs):
        ...
        for list_index, index in enumerate(list_email_indices):
            if list_index < len(list_email_indices) - 1:
                list_emails.append(
                    "\n\n".join(
                        list_paragraphs[index : list_email_indices[list_index + 1]]
                    )
                )
        return list_emails
        ...
        for index, paragraph in enumerate(list_paragraphs):
            ...
            for line in list_lines:
                if re.match(RegexRules().email_seperator_regex, line.strip().lower()):
                    ...
                    break
                list_first_words.append([word.strip() for word in line.split(" ")][0])
    ...
    def is_signature(self, text):
        ...
        list_words = [
            word
            for word in list_words
            if not re.match(RegexRules().email_address, word)
        ]
        # Remove numbers
    ...
    def remove_signature(self, email):
        ...
        if len(list_paragraphs) > 0:
            ...
            pass
        return {"email": "\n\n".join(list_paragraphs), "name": name}
