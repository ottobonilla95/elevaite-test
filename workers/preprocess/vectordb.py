...
async def recreate_collection(collection=None, size=1536, distance=Distance.COSINE):
    ...
    await qdrant_conn.recreate_collection( collection_name=collection,vectors_config=get_vector_params(size=size, distance=distance))
...
async def insert_records(collection=None, payload_with_contents=None):
    ...
    for payload in payload_with_contents:
        if (payload["page_content"]):
            ...
            response = openai.Embedding.create(input=payload["page_content"], engine=embedding_model)
