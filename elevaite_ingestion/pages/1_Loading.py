...
LOAD_STATUS_JSON_PATH = os.path.join(path, "stage/load_stage/stage_1_status.json")
LOAD_SCRIPT_PATH = os.path.join(path, "stage/load_stage/load_main.py")
...
if selected_source == "AWS S3 Bucket":
    ...
    if st.button("Save Loading Configuration"):
        if "loading" not in config:
            config["loading"] = {}
        if "sources" not in config["loading"]:
            config["loading"]["sources"] = {}
        if "s3" not in config["loading"]["sources"]:
            config["loading"]["sources"]["s3"] = {}
    ...
    if st.button("Save Loading Configuration"):
        ...
        if "sources" not in config["loading"]:
            config["loading"]["sources"] = {}
        if "local" not in config["loading"]["sources"]:
            config["loading"]["sources"]["local"] = {}
...
# st.markdown('<div class="file-uploader-box">', unsafe_allow_html=True)
...
if uploaded_files:
    os.makedirs(input_directory, exist_ok=True)  # Ensure directory exists
...
# Show "Run Loading Pipeline" button after saving the configuration
if st.session_state.loading_config_saved:
    st.markdown("---")
    if st.button("🚀 Run Loading Pipeline"):
        st.info("Executing loading pipeline... Please wait.")
        result = run_pipeline(LOAD_SCRIPT_PATH, LOAD_STATUS_JSON_PATH, "load")
        st.success("✅ Loading pipeline executed.")

# Show **Loading Pipeline Execution Status**
