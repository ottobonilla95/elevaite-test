...
PARSE_STATUS_JSON_PATH = os.path.join(path, "stage/parse_stage/stage_2_status.json")
PARSE_SCRIPT_PATH = os.path.join(path, "stage/parse_stage/parse_main.py")
...
# Sidebar: Show current configuration
st.sidebar.title("Current Configuration - Parsing")
st.sidebar.subheader(f"Parsing Mode: {default_mode.capitalize()}")
st.sidebar.json(config.get("parsing", {}))  # 🔹 Display the full parsing config

# New parsing mode options
...
if selected_mode:
    config["parsing"]["default_mode"] = selected_mode

    if selected_mode == "custom_parser":
        st.sidebar.subheader("Custom Parser Settings")

        # Get available parsers from config.json
        ...
        if selected_parser != "Select Parser":
            ...
            # Save custom parser settings
            if st.button("Save Custom Parser Configuration"):
                config["parsing"]["custom_parser_selection"] = {
                    "parser": selected_parser,
                    "tool": selected_tool if selected_tool else None
                }
                save_config(config)
                st.success("✅ Custom parser settings saved successfully!")
                st.session_state.parser_config_saved = True
                st.sidebar.json(config.get("parsing", {}))  # 🔹 Update sidebar immediately
...
# Show "Run Parser Pipeline" button only after saving configuration
if st.session_state.parser_config_saved:
    st.markdown("---")
    if st.button("🚀 Run Parser Pipeline"):
        st.info("Executing parser pipeline... Please wait.")
        parse_status = run_pipeline(PARSE_SCRIPT_PATH, PARSE_STATUS_JSON_PATH, "parser")
