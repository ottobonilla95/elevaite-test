...
@pytest.mark.asyncio
@patch("elevaitelib.orm.db.models.Role", new=models.Role)
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_create_role_with_valid_access_token_not_in_redis_cache_and_authenticating_user_is_superadmin(
    ...
    response = await client.request(
        method="POST", url="/roles/", headers=headers, json=create_role_payload
    )
    ...
    expected_response_payload = {
        ...
        "permissions": permission_schemas.AccountScopedRBACPermission.parse_obj(
            create_role_payload["permissions"]
        ).dict(),
        "created_at": ANY,
        "updated_at": ANY,
    }
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_create_role_with_valid_access_token_in_redis_cache_and_authenticating_user_is_superadmin(
    mock_requests, mock_redis_class, client, setup_initial_data
):
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_create_role_with_duplicate_role_name_and_authenticating_user_is_superadmin(
    mock_requests, mock_redis_class, client, setup_initial_data
):
    ...
    expected_response_payload = {
        "detail": f"A role with name - '{create_role_payload['name']}' - already exists"
    }
    assert actual_response_payload == expected_response_payload
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_create_role_with_invalid_or_expired_access_token(
    mock_requests, mock_redis_class, client, setup_initial_data
):
...
@patch("elevaitelib.orm.db.models.Role", new=models.Role)
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_create_role_with_valid_access_token_and_authenticating_user_is_not_superadmin(
...
@patch("elevaitelib.orm.db.models.Role", new=models.Role)
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_create_role_with_valid_access_token_and_authenticating_user_does_not_exist_in_db(
    mock_redis_class, client, setup_initial_data
):
