    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                json={
                    "module_name": f"rbac/{role_name}_role",
                    "rego_code": rego_code
                },
                headers={
            ...
            if response.status_code == 200:
                ...
                print()
                print("Next steps:")
                print(f"1. Assign the role to a user:")
                print(f"   POST /api/rbac/role-assignments")
                print(f"   {{")
                print(f"     \"user_id\": 1,")
                print(f"     \"role\": \"{role_name}\",")
                print(f"     \"resource_type\": \"{resource_type}\",")
                print(f"     \"resource_id\": \"your-resource-id\"")
                print(f"   }}")
                print()
                print(f"2. Test authorization:")
                print(f"   POST /api/authz/check_access")
                print(f"   {{")
                print(f"     \"user_id\": 1,")
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  Create role:")
        print("    python scripts/create_role.py <role_name> <actions> [--resource-type TYPE] [--description DESC]")
        print()
        print("  Examples:")
        print("    python scripts/create_role.py analyst 'view_project,export_data,view_reports'")
        print("    python scripts/create_role.py developer 'create_workflow,edit_workflow' --resource-type workflow")
        print()
    ...
    if len(sys.argv) < 3:
        ...
        print("Example: python scripts/create_role.py analyst 'view_project,export_data'")
        sys.exit(1)
