...
async def remove_biometric_device(
    ...
    result = await session.execute(
        select(BiometricDevice).where(
            BiometricDevice.id == device_id, BiometricDevice.user_id == user_id
        )
    )
    ...
    result = await session.execute(
        select(BiometricDevice).where(
            BiometricDevice.user_id == user_id, BiometricDevice.is_active
        )
    )
...
async def toggle_biometric_setting(
    ...
    if enabled is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="enabled field is required"
        )
