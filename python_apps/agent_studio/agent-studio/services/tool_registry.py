...
class ToolRegistry:
    ...
    def _load_local_tools(self, db: Session):
        ...
        local_tools = crud.get_tools(
            db, tool_type="local", is_active=True, is_available=True
        )
        ...
        for tool in local_tools:
            try:
                ...
                crud.update_tool(
                    db, tool.tool_id, 
                    schemas.ToolUpdate(is_available=False)
                )
    ...
    def get_tools_by_category(self, category_name: str, db: Session) -> List[models.Tool]:
        ...
        return [
            ...
            if tool.category_id == category.category_id
        ]
    ...
    def execute_tool(
        ...
        try:
            if tool.tool_type == "local":
                ...
                # Phase 2: Implement MCP tool execution
                raise NotImplementedError("MCP tool execution will be implemented in Phase 2")
            else:
    ...
    def register_local_tool(
        ...
        **kwargs
        ...
        if category_name:
            category = crud.get_tool_category_by_name(db, category_name)
            if not category:
                category = crud.create_tool_category(
                    db, schemas.ToolCategoryCreate(name=category_name)
                )
