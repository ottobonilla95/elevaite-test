...
def upgrade() -> None:
    ...
    op.create_table('deterministic_workflow_steps',
        sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
        sa.Column('step_id', postgresql.UUID(as_uuid=True), nullable=False, default=sa.text('gen_random_uuid()')),
    ...
    op.create_index(op.f('ix_deterministic_workflow_steps_execution_id'), 'deterministic_workflow_steps', ['execution_id'])
    op.create_index(op.f('ix_deterministic_workflow_steps_step_type'), 'deterministic_workflow_steps', ['step_type'])
    op.create_index(op.f('ix_deterministic_workflow_steps_status'), 'deterministic_workflow_steps', ['status'])
    op.create_index(op.f('ix_deterministic_workflow_steps_start_time'), 'deterministic_workflow_steps', ['start_time'])


def downgrade() -> None:
    # Drop indexes
    op.drop_index(op.f('ix_deterministic_workflow_steps_start_time'), table_name='deterministic_workflow_steps')
    op.drop_index(op.f('ix_deterministic_workflow_steps_status'), table_name='deterministic_workflow_steps')
    op.drop_index(op.f('ix_deterministic_workflow_steps_step_type'), table_name='deterministic_workflow_steps')
    op.drop_index(op.f('ix_deterministic_workflow_steps_execution_id'), table_name='deterministic_workflow_steps')
