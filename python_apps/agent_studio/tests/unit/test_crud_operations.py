...
class TestCrudOperations:
    ...
    def test_get_prompt_by_unique_label(self, test_db_session):
        """Test retrieving a prompt by unique label."""
        prompt_data = DEFAULT_PROMPTS[0]
        prompt_create = schemas.PromptCreate(
            prompt_label=prompt_data.prompt_label,
            prompt=prompt_data.prompt,
        ...
        # Test retrieval by unique label
        retrieved_prompt = crud.get_prompt_by_unique_label(
            db=test_db_session, 
            unique_label=prompt_data.unique_label
        )
        assert retrieved_prompt is not None
        assert retrieved_prompt.unique_label == prompt_data.unique_label
