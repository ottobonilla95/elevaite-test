...
FULL_SCHEMA = {
    ...
    "customers": [
        "SR Customer Account Number", "Column1", "SR Address Line 1", "SR Address Line 4",
        "SR City", "SR State", "SR Postal Code", "SR Country"
    ],
    "tasks": [
...
def read_csv_robust(file_path):
    ...
    # Strategy 2: Skip bad lines and use python engine
    try:
    ...
    # Strategy 3: Use maximum columns approach
    try:
        logger.info(f"Trying max columns approach for {filename}")
        # First, scan file to find maximum number of columns
        max_cols = 0
        ...
        # Create column names for max columns found
        ...
        if not df.empty:
            logger.info(f"Max columns approach successful for {filename} ({max_cols} columns)")
            return df
    except Exception as e:
        logger.warning(f"Max columns approach failed for {filename}: {str(e)}")
    
    # Strategy 4: Last resort - read as text and try to parse manually
    try:
        logger.info(f"Trying manual parsing for {filename}")
        with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
            lines = f.readlines()
        ...
        # Use first line as headers, pad subsequent lines to match
        ...
        if not df.empty:
            logger.info(f"Manual parsing successful for {filename}")
            return df
            
    except Exception as e:
        logger.warning(f"Manual parsing failed for {filename}: {str(e)}")
...
def transform_excel_to_model(file_path):
    """
    Enhanced version that handles malformed CSVs and only rejects for truly critical missing columns
    """
    ...
    logger.info(f"Processing file with {len(df)} rows and columns: {list(df.columns)}")
    ...
    try:
        ...
        data = {
            ...
            "customers": customers_df.rename(columns={
                ...
                "SR Address Line 1": "address_line1", 
                "SR Address Line 4": "address_line2",
                "SR City": "city",
                "SR State": "state",
                "SR Postal Code": "postal_code",
                "SR Country": "country"
            }).drop_duplicates(),
        ...
        for table_name, table_df in data.items():
            if not table_df.empty:
                logger.info(f"✅ {table_name}: {len(table_df)} rows processed")
