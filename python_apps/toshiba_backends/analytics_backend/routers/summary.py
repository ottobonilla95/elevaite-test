...
def build_date_filter(start_date: Optional[str], end_date: Optional[str]):
    """Build date filter clause - NOW USING CLOSED_DATE"""
    if start_date and end_date:
        return f"AND sr.closed_date BETWEEN '{start_date}' AND '{end_date}'"
    elif start_date:
        return f"AND sr.closed_date >= '{start_date}'"
    elif end_date:
        return f"AND sr.closed_date <= '{end_date}'"
    else:
        return ""
...
def get_enhanced_summary_metrics(
    ...
    try:
        ...
        if fst_id:
            ...
            if date_filter:
                query = f"SELECT COUNT(*) as total FROM service_requests sr WHERE closed_date IS NOT NULL {date_filter}"
            else:
                query = "SELECT COUNT(*) as total FROM service_requests WHERE closed_date IS NOT NULL"
        ...
        if fst_id:
            date_query = f"""
                SELECT 
                    MIN(sr.closed_date) as min_date,
                    MAX(sr.closed_date) as max_date,
                    COUNT(DISTINCT DATE(sr.closed_date)) as unique_days
                FROM service_requests sr
                LEFT JOIN tasks fst_tasks ON sr.sr_number = fst_tasks.sr_number
                LEFT JOIN fst_technicians ft ON (
                    fst_tasks.assignee_name = ft.full_name OR
            ...
            if date_filter:
                date_query = f"""
                    SELECT 
                        MIN(closed_date) as min_date,
                        MAX(closed_date) as max_date,
                        COUNT(DISTINCT DATE(closed_date)) as unique_days
                    FROM service_requests sr
                    WHERE closed_date IS NOT NULL {date_filter}
                """
            else:
                date_query = """
                    SELECT 
        ...
        return {
            ...
            "_data_source": "closed_date_based",
            ...
            "_avg_resolution_days_calculated": round(avg_resolution_days, 1)
        }
...
def get_service_request_trends(
    ...
    """Trends with FST filtering support AND date filtering - USING CLOSED_DATE"""
    try:
        date_filter = build_date_filter(start_date, end_date)
        print(f"DEBUG: date_filter: {date_filter}")
        ...
        if fst_id:
            query = f"""
                SELECT 
                    DATE_TRUNC('month', sr.closed_date)::date as period,
                    COUNT(DISTINCT sr.sr_number) as count
                FROM service_requests sr
                LEFT JOIN tasks fst_tasks ON sr.sr_number = fst_tasks.sr_number
                LEFT JOIN fst_technicians ft ON (
            ...
            if date_filter:
                query = f"""
                    SELECT 
                        DATE_TRUNC('month', closed_date)::date as period,
                        COUNT(*) as count
                    FROM service_requests sr
                    WHERE closed_date IS NOT NULL
                    {date_filter}
...
def get_machine_distribution(
    ...
    """Machine distribution with FST filtering support AND date filtering - USING CLOSED_DATE"""
    try:
        date_filter = build_date_filter(start_date, end_date)
        print(f"DEBUG: date_filter: {date_filter}")
...
def get_severity_distribution(
    ...
    """Severity distribution with FST filtering support AND date filtering - USING CLOSED_DATE"""
    try:
        date_filter = build_date_filter(start_date, end_date)
        print(f"DEBUG: date_filter: {date_filter}")
