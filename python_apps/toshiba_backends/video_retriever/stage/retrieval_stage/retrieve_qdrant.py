...
def process_match(match) -> Dict:
    ...
    filename = payload.get("video_filename")
    page_info = payload.get("page_range")
    context_header = payload.get("contextual_header")
    timestamp_indices = payload.get("timestamp_indices")
    image_path = None
    return {
        "chunk_id": chunk_id,
            ...
            filters = models.Filter(
                should=[
                    models.FieldCondition(
                        key="filename", match=models.MatchText(text=mt)
                    )
                    for mt in machine_types
                ]
                + [
                    models.FieldCondition(
                        key="table_filename", match=models.MatchText(text=mt)
                    )
                    for mt in machine_types
                ]
            )
        ...
        filters = models.Filter(
            should=[
                ...
                models.Filter(
                    should=[
                        models.FieldCondition(
                            key="filename", match=models.MatchAny(any=machine_types)
                        )
                    ]
                ),
            ]
                ...
                + [
                    models.Filter(
                        must=[
                            ...
                            models.FieldCondition(
                                key="chunk_text", match=models.MatchText(text=kw)
                            ),
                            models.Filter(
                                should=[
                                    models.FieldCondition(
...
def multi_strategy_search(
    query: str, top_k: int = 30, machine_types: Optional[List[str]] = None
) -> List[Dict]:
    seen_ids = set()
    results = []
    ...
    semantic_results = retrieve_chunks_semantic(
        query, top_k=top_k, machine_types=machine_types
    )
    ...
    # print("Keywords: ", keywords)
    sparse_matches = retrieve_by_keywords(
        keywords, top_k=10, machine_types=machine_types
    )
