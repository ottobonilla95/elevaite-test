...
FULL_SCHEMA = {
    ...
    "customers": [
        "SR Customer Account Number", "SR Customer Name", "SR Address Line 1", "SR Address Line 4", "SR City", "SR State", "SR Country", "SR Postal Code"
    ],
    "tasks": [
...
def transform_excel_to_model(file_path):
    ...
    try:
        ...
        # Combine Customer Account Number and Address line 1 to create a unique customer ID
        new_column = service_requests_df['SR Customer Account Number'].astype(str) + '|' + service_requests_df['SR Address Line 1'].astype(str) + '|' + service_requests_df['SR Postal Code'].astype(str)
        hashed_ids = new_column.apply(lambda x: hashlib.md5(x.encode('utf-8')).hexdigest())
        service_requests_df['Customer Unique ID'] = hashed_ids

        customers_df = fill_missing_columns_relaxed(df.copy(), "customers")
        customers_df = truncate_long_fields(customers_df)
        customers_df['Customer Unique ID'] = hashed_ids
