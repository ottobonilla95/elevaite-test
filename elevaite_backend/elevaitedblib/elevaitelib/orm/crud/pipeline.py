...
def get_pipelines_of_application(db: Session, application_id: int, skip: int = 0, limit: int = 100) -> list[models.Pipeline]:
    return db.query(models.Pipeline).filter(models.Pipeline.applicationId == application_id).offset(skip).limit(limit).all()
