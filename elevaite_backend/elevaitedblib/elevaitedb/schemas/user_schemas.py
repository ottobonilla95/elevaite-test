...
class UserPatchRequestDTO(BaseModel):
   ...
   def check_not_all_none(cls, values):
      ...
      if all(value is None for value in values.values()):
         print(f"Inside PATCH /users/user_id - UserPatchRequestDTO schema validation")
...
class UserListDTO(BaseModel):
   ...
   def validate_user_ids(cls, v : List[UUID]) -> List[UUID]:
      ...
      if len(v) < 1 or len(v) > 50:
         print(f"Inside UserListDTO schema validation")
         raise ValueError('The list of user IDs must have length between 1 and 50 (inclusive)')
      return v
...
class UserProfileDTO(BaseModel):
   id: UUID = Field(...)
   organization_id: UUID = Field(..., description="The ID of the org to be assigned to the user")
   ...
   account_memberships: List[UserAccountMembershipInfo] = Field(..., description="List of accounts the user is part of and their roles within those accounts")
...
class AccountUserListItemDTO(OrgUserListItemDTO):
   ...
   roles: List[RoleSummaryDTO] = Field(..., description="List of RoleSummaryDTO objects scoped to account")
