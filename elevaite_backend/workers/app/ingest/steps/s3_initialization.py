...
from ...interfaces import S3IngestData, ServiceNowIngestData, isS3IngestData
...
class S3Initialization(BaseIngestStep):
    ...
    def run(self):
        if not isS3IngestData(self.data):
        ...
        s3url = S3Url(self.data.url)
        _instance_registry.s3url = s3url
