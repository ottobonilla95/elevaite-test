...
async def validate_patch_organization(
   ...
   try:
      ...
      org_to_patch = db.query(Organization).filter(Organization.id == org_id).first()
      if not org_to_patch:
...
async def validate_get_organization(
   ...
   try:
      ...
      org_id = os.getenv("ORGANIZATION_ID", None)
      print(f'org_id = {org_id}')
      db_org_exists = db.query(exists().where(Organization.id == org_id)).scalar()
...
async def validate_get_org_users(
   ...
   try:
      ...
      user_admin_accounts_exist = db.query(User_Account).join(
