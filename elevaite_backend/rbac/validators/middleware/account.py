...
async def validate_patch_account(
   ...
   try:
      ...
      logged_in_user_id_subquery = select(User.id).where(User.email == user_email).limit(1).as_scalar().label("logged_in_user_id") 
      user_is_superadmin_subquery = select(User.is_superadmin).where(User.email == user_email).as_scalar().label("user_is_superadmin")
      
      user_is_admin_subquery = select(exists().where(and_( 
         User_Account.user_id == logged_in_user_id_subquery,
...
async def validate_get_account_user_list_or_profile(
   ...
   try:
      ...
      user_account_association = db.query(User_Account).filter_by(user_id=logged_in_user.id, account_id=account_id).first()
...
async def validate_patch_account_admin_status(
   ...
   try:
      ...
      logged_in_user_account_association = db.query(User_Account).filter(
         User_Account.user_id == logged_in_user.id,
         User_Account.account_id == account_id
      ).first()
