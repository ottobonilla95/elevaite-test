...
from elevaitedb.schemas.role_schemas import ProjectScopedPermissions
from app.utils.deps import get_db  
from app.errors.api_error import ApiError
from elevaitedb.schemas.project_schemas import ProjectCreationRequestDTO
...
async def validate_get_project(
   ...
   try:
      ...
      user_project_association_exists = db.query(exists().where(
         and_(
            User_Project.user_id == logged_in_user.id,
      ...
      if project.parent_project_id and not is_user_project_association_till_root(db=db, starting_project_id=project.parent_project_id, user_id=logged_in_user.id):
         print(f"logged-in user - '{logged_in_user.id}' - is not assigned to all projects in the project hierarchy of project - '{project_id}'")
         raise ApiError.forbidden(f"you are not assigned to all projects in the project hierarchy of project - '{project_id}'")
...
async def validate_get_projects( # When associations cascade deleted, this will perform as expected. When project is disabled, can still see them, but cant perform actions.
   ...
   try:
      ...
      # If parent_project_id is provided, check for parent project existence
      if parent_project_id:
         parent_project_exists = db.query(exists().where(Project.id == parent_project_id)).scalar()
      ...
      if parent_project_id and not is_user_project_association_till_root(db=db, starting_project_id=parent_project_id, user_id=logged_in_user.id):
         print(f"logged-in user - '{logged_in_user.id}' - is not assigned to all projects in the project hierarchy of project - '{parent_project_id}'")
         raise ApiError.forbidden(f"you are not assigned to all projects in the project hierarchy of project - '{parent_project_id}'")
...
async def validate_get_project_user_list(
   ...
   try:
      ...
      user_account_association = db.query(User_Account).filter(
         User_Account.user_id == logged_in_user.id, User_Account.account_id == account.id
      ).first()
      ...
      # Check account disabled, project membership, and ancestor project disabled for regular users
...
async def validate_patch_project(
   ...
   try:
      ...
      if user_account:
         if user_account.is_admin: # and not project.is_disabled: # cte flag
            return {"logged_in_user_id" : logged_in_user.id,
                     "is_superadmin" : False,
                     "is_admin" : True,
                     "is_owner" : None,
         ...
         if logged_in_user.id == project.project_owner_id: 
            if not is_user_project_association_till_root(db=db, starting_project_id=project.id, user_id=logged_in_user.id):
      ...
      raise ApiError.forbidden(f"you are not assigned to account - '{account.id}' - and dont have superadmin privileges to patch project - '{project_id}'")
   except HTTPException as e:
      db.rollback()
...
async def extract_and_validate_post_project_data(project_creation_payload: ProjectCreationRequestDTO = Body(...),
                                                 user_email: str = Depends(validate_token),
                                                 db: Session = Depends(get_db)):
...
async def validate_post_project( # cascade delete on associations will make this work as intended. disabled status of account and projects need to be checked
   ...
   try: 
      ...
      if parent_project_id:
         parent_project = db.query(Project).filter(Project.id == parent_project_id).first()
      ...
      logged_in_user_account = db.query(User_Account).filter(
         User_Account.user_id == logged_in_user.id,
         User_Account.account_id == account_id
      ).first()

      # Validate logged-in user's membership in the account
      if not logged_in_user_account:
      ...
      # account admin case
      if logged_in_user_account.is_admin:
      ...
      if parent_project_id:
         parent_project_association = db.query(User_Project).filter(
            User_Project.user_id == logged_in_user.id,
            User_Project.project_id == parent_project_id
         ).first()

         if not parent_project_association:
            print(f"logged-in user - '{logged_in_user.id}' - is not assigned to project - '{parent_project_id}'")
         ...
         if not is_user_project_association_till_root(db=db, starting_project_id=parent_project_id, user_id=logged_in_user.id):
            print(f"logged-in user - '{logged_in_user.id}' - is not assigned to all projects in the project hierarchy of project - '{parent_project_id}'")
            raise ApiError.forbidden(f"you are not assigned to all projects in the project hierarchy of parent project - '{parent_project_id}'")

      # logged-in user is neither superadmin or admin at this point, and belongs to account_id. 
      # so conclude permissions based on account-scoped role based access which is common regardless of parent-project presence, and then check for project-scoped permission on parent-project if exists
      role_based_access_exists = db.query(
         db.query(Role)
         .join(Role_User_Account, Role_User_Account.role_id == Role.id)
         .filter(
            Role_User_Account.user_account_id == logged_in_user_account.id,
            Role.permissions['Project']['CREATE']['action'].astext == 'Allow'
         )
         .exists()
      ).scalar()

      if not role_based_access_exists:
         print(f"INSIDE POST /projects validate_post_project: logged-in user -'{logged_in_user.id}'- is not a superadmin/admin and also does not have role-based access permission to create projects in account - {account_id}")
         raise ApiError.forbidden(f"you do not have superadmin/admin privileges or role-based access to create projects in account - '{account_id}'")
      
      if parent_project_id and cast(ProjectScopedPermissions, parent_project_association.permission_overrides):
         # Fetch permission overrides for the parent project
         # user_project_overrides = ProjectScopedPermissions.model_validate(parent_project_association.permission_overrides)
         user_project_overrides = ProjectScopedPermissions.parse_obj(parent_project_association.permission_overrides)
         
         # print(f'user_project_overrides.Project = {user_project_overrides.Project}')
         # print(f'user_project_overrides.Project.CREATE = {user_project_overrides.Project.CREATE}')
         # print(f'user_project_overrides.Project.CREATE.action = {user_project_overrides.Project.CREATE.action}')
         # Check if there's a specific "Deny" for project creation in permission overrides
         if user_project_overrides and user_project_overrides.Project.CREATE.action == 'Deny': 
            print(f"INSIDE POST /projects validate_post_project: user -{logged_in_user.id}- has project-specific override permissions which restrict creation of subprojects under parent project -{parent_project_id}")
...
async def validate_assign_or_deassign_users_in_project(
   ...
   try:
      ...
      logged_in_user = db.query(User).filter(User.email == email).first()
      if not logged_in_user:
         print(f'INSIDE PATCH /projects/{project_id}/users - validate_assign_project: logged-in user with email - "{email}" - not found')
         raise ApiError.unauthorized("user is unauthenticated")
      
      # Fetch project by project_id
      project = db.query(Project).filter(Project.id == project_id).first()
      if not project:
         print(f'INSIDE PATCH /projects/{project_id}/users - validate_assign_project: Project - "{project_id}" - not found.')
         raise ApiError.notfound(f"project - '{project_id}' - not found")
      
      # Fetch account associated with the project
      account = db.query(Account).filter(Account.id == project.account_id).first()
      if not account:
         print(f'INSIDE PATCH /projects/{project_id}/users - validate_assign_project: associated account - "{project.account_id}" - for project - "{project_id}" - not found')
         raise ApiError.notfound(f"account - '{project.account_id}' - not found")

      if account.is_disabled:
         print(f'INSIDE PATCH /projects/{project_id}/users - validate_assign_project: associated account - "{project.account_id}" - is disabled')
         raise ApiError.forbidden(f"account - '{project.account_id}' - is disabled")
      ...
      if not logged_in_user_account:
         print(f'INSIDE PATCH /projects/{project_id}/users - validate_assign_project: logged-in user -"{logged_in_user.id}"- is not assigned to project account - {project.account_id}')
         raise ApiError.forbidden(f"you are not assigned to account - '{project.account_id}'")
