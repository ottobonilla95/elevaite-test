...
async def test_get_organization_with_existing_organization_and_valid_access_token_not_in_redis_cache_and_authenticating_user_exists_in_db(mock_requests, mock_redis_class, client, setup_initial_data):
    
   # Mock the GoogleIDP reques.get method
   mock_response = MagicMock()
   mock_response.json.return_value = get_user_email_response_with_success(email = db_data["user3"]["email"])
   ...
   mock_redis_instance.connection.setex.assert_called_once_with(token, 60 * 60, db_data["user3"]["email"])
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_organization_with_existing_organization_and_valid_access_token_in_redis_cache_and_authenticating_user_exists_in_db(mock_requests, mock_redis_class, client, setup_initial_data):
...
@pytest.mark.asyncio
async def test_get_organization_with_no_access_token(client, setup_initial_data):
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_organization_with_invalid_or_expired_access_token(mock_requests, mock_redis_class, client, setup_initial_data):
...
@pytest.mark.asyncio
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_get_organization_with_existing_organization_and_valid_access_token_and_authenticating_user_does_not_exist_in_db(mock_redis_class, client, setup_initial_data):
...
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_get_organization_with_non_existent_organization_and_valid_access_token_and_authenticating_user_exists_in_db(mock_redis_class, mock_os_getenv, client, setup_initial_data):
