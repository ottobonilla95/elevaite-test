...
async def test_get_roles_with_valid_access_token_not_in_redis_cache_and_authenticating_user_is_superadmin(mock_requests, mock_redis_class, client, setup_initial_data):
   ...
   expected_response_payload = [{
      ...
      "permissions": permission_schemas.AccountScopedRBACPermission.parse_obj(db_data["role1"]["permissions"]).dict(),
      "created_at" : ANY,
      "updated_at": ANY
   },
   {
      ...
      "permissions": permission_schemas.AccountScopedRBACPermission.parse_obj(db_data["role2"]["permissions"]).dict(),
      "created_at" : ANY,
      "updated_at": ANY
   },{
      "id": db_data["role3"]["id"],
      "name": db_data["role3"]["name"],
      "permissions": permission_schemas.AccountScopedRBACPermission.parse_obj(db_data["role3"]["permissions"]).dict(),
      "created_at" : ANY,
      "updated_at": ANY
   }]
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_roles_with_valid_access_token_not_in_redis_cache_and_authenticating_user_is_account_admin(mock_requests, mock_redis_class, client, setup_initial_data):
   ...
   token = headers["Authorization"].split(" ")[1]
   # Verify the exact call to setex
   mock_redis_instance.connection.setex.assert_called_once_with(token, 60 * 60, db_data["user2"]["email"])
   # Assert the call and return value of connection.get
   mock_redis_instance.connection.get.assert_called_once_with(token)
   assert mock_redis_instance.connection.get(token) is None
   # Assert the call of GoogleIDP requests.get
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_roles_with_valid_access_token_in_redis_cache_and_authenticating_user_is_superadmin(mock_requests, mock_redis_class, client, setup_initial_data):
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_roles_with_valid_access_token_in_redis_cache_and_authenticating_user_is_account_admin(mock_requests, mock_redis_class, client, setup_initial_data):
...
@pytest.mark.asyncio
async def test_get_roles_with_no_access_token(client, setup_initial_data):
...
@patch("rbac_api.auth.impl.RedisSingleton")
@patch("rbac_api.auth.idp.impl.requests")
async def test_get_roles_with_invalid_or_expired_access_token(mock_requests, mock_redis_class, client, setup_initial_data):
...
@pytest.mark.asyncio
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_get_roles_with_valid_access_token_and_authenticating_user_does_not_exist_in_db(mock_redis_class, client, setup_initial_data):
...
@pytest.mark.asyncio
@patch("rbac_api.auth.impl.RedisSingleton")
async def test_get_roles_with_non_superadmin_and_non_account_admin_authenticating_user(mock_redis_class, client, setup_initial_data):
   ...
   expected_response_payload = {"detail": f"logged-in user - '{db_data['user3']['id']}' - does not have superadmin/account-admin privileges to read all roles"}
