from fastapi import Depends, Query, Body, Path, Depends, HTTPException
...
async def validate_get_user_profile(
   ...
   try:
      ...
      user_association_exists = db.query(exists().where(and_(models.User_Account.user_id == user_id, models.User_Account.account_id == account_id))).scalar()
...
async def validate_update_project_permission_overrides(
   ...
   try:
      ...
      user_account_association = db.query(models.User_Account).filter(models.User_Account.user_id == user_to_patch.id, models.User_Account.account_id == project.account_id).first()
      if not user_account_association:
      ...
      # Check User_Account association for logged-in user and admin status
      logged_in_user_account_association = db.query(models.User_Account).filter(models.User_Account.user_id == logged_in_user.id, models.User_Account.account_id == project.account_id).first()
      if not logged_in_user_account_association:
      ...
      if not role_based_access_exists:
         print(f"INSIDE PATCH /users/{user_id}/projects/{project_id} validate_update_project_permission_overrides dependency: logged-in user - '{logged_in_user.id}' - is not a superadmin/admin and does not have account-specific role-based access permissions to perform actions on or within project resources due to denied project 'READ' permissions in account - '{account.id}'")
      ...
      if project.project_owner_id == logged_in_user.id:
         if not is_user_project_association_till_root(db=db, starting_project_id=project.id, user_id=logged_in_user.id):
            print(f"INSIDE PATCH /users/{user_id}/projects/{project_id} validate_update_project_permission_overrides dependency: logged-in user - '{logged_in_user.id}' - is not assigned to one or more projects in the project hierarchy of project - '{project_id}'")
            raise ApiError.forbidden(f"you are not assigned to one or more projects in the project hierarchy of project - '{project_id}'")
         return validation_info
...
async def validate_get_project_permission_overrides(
   ...
   try:
      ...
      user_account_association = db.query(models.User_Account).filter(models.User_Account.user_id == user.id, models.User_Account.account_id == project.account_id).first()
      if not user_account_association:
      ...
      if not role_based_access_exists:
         print(f"INSIDE GET /users/{user_id}/projects/{project_id} validate_get_project_permission_overrides dependency: logged-in user - '{logged_in_user.id}' - is not a superadmin/admin and does not have account-specific role-based access permissions to perform actions on or within project resources due to denied project 'READ' permissions in account - '{account.id}'")
         raise ApiError.forbidden(f"you are not a superadmin/admin and you do not have account-specific role-based access permissions to perform actions on or within project resources due to denied Project - 'READ' permissions in account - '{account.id}'")
      
      # Check User_Project association for logged-in user and check project ownership (he should not be able to read resource if he is not part of project but was owner), or if user is logged-in user
      if project.project_owner_id == logged_in_user.id or logged_in_user.id == user.id:
         if not is_user_project_association_till_root(db=db, starting_project_id=project.id, user_id=logged_in_user.id):
            print(f"INSIDE GET /users/{user_id}/projects/{project_id} validate_get_project_permission_overrides dependency: logged-in user - '{logged_in_user.id}' - is not assigned to one or more projects in the project hierarchy of project - '{project_id}'")
            raise ApiError.forbidden(f"you are not assigned to one or more projects in the project hierarchy of project - '{project_id}'")
         return validation_info
