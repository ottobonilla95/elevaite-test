...
def validate_create_apikey_factory(target_model_class : Type[models.Base], target_model_action_sequence: tuple[str, ...]):
   async def validate_create_apikey(
      ...
      try: 
         ...
         db.rollback()
         pprint(f'API error in POST /apikeys/ - validate_create_apikey middleware : {e}')
         raise e
...
def validate_get_apikeys_factory(target_model_class : Type[models.Base], target_model_action_sequence: tuple[str, ...]):
   async def validate_get_apikeys(
      ...
      try: 
         ...
         db.rollback()
         pprint(f'API error in GET /apikeys/ - validate_get_apikeys middleware : {e}')
         raise e
      except SQLAlchemyError as e:
         db.rollback()
...
def validate_get_apikey_factory(target_model_class : Type[models.Base], target_model_action_sequence: tuple[str, ...]):
   async def validate_get_apikey(
      ...
      # The params below are required for pydantic validation even when unused
      project_id: UUID = Path(..., description="The ID of the project under which apikey is read"),
      ...
      try: 
         ...
         or \
         (logged_in_user_project_association and logged_in_user_project_association.is_admin):
            return validation_info
         ...
         db.rollback()
         pprint(f'API error in GET /apikeys/{apikey_id} - validate_get_apikey middleware : {e}')
         raise e
      except SQLAlchemyError as e:
         db.rollback()
...
def validate_delete_apikey_factory(target_model_class : Type[models.Base], target_model_action_sequence: tuple[str, ...]):
   async def validate_delete_apikey(
      ...
      # The params below are required for pydantic validation even when unused
      project_id: UUID = Path(..., description="The ID of the project under which apikey is deleted"),
      logged_in_user: models.User = Depends(AccessTokenAuthentication.authenticate), 
   ) -> dict[str, Any]:
      db: Session = request.state.db
      try: 
         ...
         db.rollback()
         pprint(f'API error in DELETE /apikeys/{apikey_id} - validate_delete_apikey middleware : {e}')
         raise e
      except SQLAlchemyError as e:
         db.rollback()
