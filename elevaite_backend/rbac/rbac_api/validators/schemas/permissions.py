# The value 'Allow/Deny' has no meaning other than for schema validation against the pydantic class - 'AccountScopedPermission'
...
# When modifying this schema, the pydantic class for AccountScopedPermission must also be updated to reflect the new schema.
...
# This is the schema used for checking project permission overrides; It only contains permissions applicable at the project level; Project ENTITY's READ action, Configuration ENTITY and Application ENTITY's READ action permissions are omitted since they are account level actions and resources.
