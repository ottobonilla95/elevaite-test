...
def is_user_project_association_till_root(db: Session, starting_project_id: UUID, user_id: UUID):
   try:
      ...
      base_cte = select(
         ...
         models.Project.id == starting_project_id
...
def delete_user_project_associations_for_subprojects(db: Session, starting_project_id: UUID, user_id_or_user_ids: Union[List[UUID], UUID]):
   try:
      ...
      cte = select(
         models.Project.id.label("project_id"),
      ).where(models.Project.id == starting_project_id).cte(name="descendants", recursive=True)
...
def delete_unrooted_user_project_associations(db: Session, user_id: UUID, list_of_starting_project_ids: List[UUID], account_id: Union[UUID, None] = None):
   try:
      ...
      cte = select(
         models.Project.id.label("project_id"),
      ).where(models.Project.id.in_(list_of_starting_project_ids)).cte(name="descendants", recursive=True)
