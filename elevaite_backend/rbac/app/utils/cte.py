...
from sqlalchemy import func, case, and_, select, exists, Integer, literal_column
...
def is_user_project_association_till_root(db: Session, starting_project_id: UUID, user_id: UUID):
   try:
      ...
      recursive_part = select(
         ...
         (ancestor_cte.c.height + 1).label('height')  # Increment height at each step
      ).select_from(
         ancestor_cte
         .join(project_alias, project_alias.id == ancestor_cte.c.parent_project_id)
         .join(User_Project, and_(
               User_Project.project_id == project_alias.id,
               User_Project.user_id == user_id
         ))
      )
      ...
      if last_project and last_project.parent_project_id is None:
         return True  # Reached the root project
      else:
         return False  # Did not reach the root project
   except SQLAlchemyError as e:
      db.rollback()  
      print(f"INSIDE check_project_association_till_root method, DB Error : {e}")
      raise e
   except Exception as e:
      db.rollback()  # Roll back the transaction on any unexpected error
      print(f"INSIDE check_project_association_till_root method, unexpected error : {e}")
      raise e
def delete_user_project_associations_for_subprojects(db: Session, starting_project_id: UUID, user_id_or_user_ids: Union[List[UUID], UUID]):
   try:
      ...
      cte = select(
         Project.id.label("project_id"),
      ).where(Project.id == starting_project_id).cte(name="descendants", recursive=True)
      ...
      if isinstance(user_id_or_user_ids, list):
         ...
         delete_query = db.query(User_Project).filter(
               User_Project.user_id.in_(user_id_or_user_ids),
               User_Project.project_id.in_(db.query(cte.c.project_id))
         )
      else:
         # If user_id_or_user_ids is a single UUID, use the `==` operator
         delete_query = db.query(User_Project).filter(
               User_Project.user_id == user_id_or_user_ids,
               User_Project.project_id.in_(db.query(cte.c.project_id))
         )
      ...
      db.rollback()  
      print(f"INSIDE delete_user_project_associations_for_subprojects method, DB Error : {e}")
      raise e
   except Exception as e:
      db.rollback()  # Roll back the transaction on any unexpected error
...
def delete_unrooted_user_project_associations(db: Session, user_id: UUID, list_of_starting_project_ids: List[UUID], account_id: UUID):
   try:
      ...
      subquery = db.query(User_Project.id)\
         .join(Project, User_Project.project_id == Project.id)\
         .filter(User_Project.user_id == user_id,
                  Project.account_id == account_id,
                  User_Project.project_id.notin_(db.query(cte.c.project_id)))\
         .subquery()
         
      # Delete those User_Project entries using the IDs obtained above
      deleted_unrooted_user_project_association_count = db.query(User_Project)\
         .filter(User_Project.id.in_(db.query(subquery.c.id)))\
