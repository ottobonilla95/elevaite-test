...
from elevaitedb.db.models import Role
from app.errors.api_error import ApiError
from pprint import pprint

def create_role(
    role_creation_dto: RoleCreationRequestDTO,
    db: Session
) -> RoleResponseDTO :
   try:
      existing_role = db.query(Role).filter(Role.name == role_creation_dto.name).first()
...
def get_roles(
   ...
   try:
      query = db.query(Role)
      roles = query.all()
      return cast(List[RoleResponseDTO], roles)
   except SQLAlchemyError as e: # group db side error as 503 to not expose actual error to client
      db.rollback()
      pprint(f'DB error in GET /roles service method : {e}')
      raise ApiError.serviceunavailable("The server is currently unavailable, please try again later.")
...
def patch_role(
   role_id: UUID,
   role_patch_req_payload: RoleUpdateRequestDTO,
   db: Session,
) -> RoleResponseDTO:
   try:
