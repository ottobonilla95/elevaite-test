...
async def create_project(
   project_creation_payload: ProjectCreationRequestDTO = Body(..., description="project creation payload"),
...
async def patch_project(
   project_patch_payload: ProjectPatchRequestDTO = Body(...),
   validation_info: dict[str, Any] = Depends(validate_patch_project) # Assuming the dependency function extracts the user ID from the access token, and performs all required validations.
      ...
      project_to_patch: Project = validation_info.get("project_to_patch", None)
      db: Session = validation_info.get("db", None)
...
async def get_projects( 
   ...
   view: Optional[ProjectView] = Query(None, description = "View mode, either - 'Flat' or 'Hierarchical'; default value is 'Flat'. When set to 'Flat', parent_project_id will not be considered"),
   ...
   type: Optional[ProjectType] = Query(None, description = "Optional filter for querying projects based on ownership, either - 'My_Projects' or 'Shared_With_Me'; if not provided then this will assume both types (all projects for superadmin/admins)"),
...
async def assign_or_deassign_users_in_project(
   user_list_dto: UserListDTO = Body(description = "payload containing user_id's to assign/deassign to/from project"),
